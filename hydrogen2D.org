...
#+RESULTS:
: Graphene 3x3 pore=1:
: [[./img/material/g-mono/3x3/pore=1.png]]
...
#    print(i, energy, time)
    print("H Distance: {:5.2f}. Energy = {:8.3f}. Calculation time: {:3.0f} min.".format(i, energy, time/60.))

def print_inprogress(i):
    print("H Distance: {:5.2f}. In progress.".format(i))

def print_unconverged(i):
    print("H Distance: {:5.2f}. Did not converge.".format(i))


# Distance of molecule's center from grahene plane
...
# Build base images
with jasp('material/g-mono/3x3/pore=1/vac=20') as calc:
    G_defect = calc.get_atoms()

HH_bond = 0.74 # H-H bond length (‎Å)

atoms =  [Atoms(G_defect) for i in range(len(heights))]
for i, h in enumerate(heights):
    H2 = Atoms([Atom('H', (0, 0, h - HH_bond/2.)),
                Atom('H', (0, 0, h + HH_bond/2.))])
    atoms[i].extend(H2)
...
atoms = [Atoms(atoms[i]) for i in range(len(heights))]
for i in range(len(heights)):
    atoms[i].set_constraint(hydrogen_constraint)

print('Unconstrained graphene, leading fixed hydrogen:')
name = 'transport/g-mono/3x3/pore=1/linear-unconst_G'
ready = True
for i, h in enumerate(heights):
...
graphene = Atoms([Atom('C', 1./2. * a3),
                  Atom('C', 1./3. * a1 + 1./3. * a2 + 1./2. * a3)],
                  cell=[a1, a2, a3])

i = 3
graphene = graphene.repeat((i,i,1))
...
atoms_free = [Atoms(atoms[i]) for i in range(len(heights))]

atoms_uncon = [Atoms(atoms[i]) for i in range(len(heights))]
for i in range(len(heights)):
    atoms_uncon[i].set_constraint(hydrogen_constraint)

atoms_con = [Atoms(atoms[i]) for i in range(len(heights))]
for i in range(len(heights)):
