...
***** DONE Non-centered, standard spacing
...
***** DONE Non-centered, wide vaccuum
...
def calculate(nsw):
    ...
    with jasp('./dft/' + name,
        ...
        try:
            calc.calculate()
            energy = atoms.get_potential_energy()/len(atoms)
            dtb.status_converged(len(atoms), energy, float(get_elapsed_time(calc)))
        except (VaspSubmitted, VaspQueued):
            dtb.status_inprogress()
            ready = False
            pass
...
Final structure calculation: In progress.
[[./img/material/g-bi/3x3/pore=1/test/nsw=10.png]]
Final structure calculation: In progress.
:END:
...
#+RESULTS:
:RESULTS:
[[./img/material/g-bi/6x6/pore=10.png]]
Failed to complete: [4, 7]
kpts=(1,1,1):
Final structure calculation: In progress.
kpts=(2,2,1):
Final structure calculation: In progress.
...
name_uncon = base_name + 'linear-const-free'
TE_uncon = []
