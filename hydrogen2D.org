...
dtb.paint_atoms(atoms, [index])
dtb.paint_atoms(atoms, neighbors, sym='O')
dtb.bp(atoms)
#+END_SRC
...
a = atoms.copy()
dtb.paint_atoms(a, edges)
dtb.print_image('presentation/group-meeting-20160706/edges', a)

a = atoms.copy()
dtb.paint_atoms(a, edges)
dtb.paint_atoms(a, island, sym="O")
dtb.print_image('presentation/group-meeting-20160706/island', a)

a = atoms.copy()
dtb.paint_atoms(a, edges)
dtb.paint_atoms(a, bad_pore, sym="O")
dtb.print_image('presentation/group-meeting-20160706/bad_pore', a)
#+END_SRC
...
** 2016-07-21: Make twodee and dtb a module named twodee
...
Functions to facilitate easy 2D material unitcell manipulation. Show what each function does.
...
for i, c in enumerate(cans):
    a = atoms.copy()
    dtb.paint_atoms(a, c, sym="O")
    dtb.write_image('movies/candidates-islands/size={1}/{0:0>4}'.format(i, size), a)
...
for ismear in ismears:
    ...
    calc = Vasp(name,
                ...
                nsw=50,            # max number of steps to relax
                atoms=atoms)
    dtb.result('vacuum ismear={0:2d}'.format(ismear), calc)
    energies.append(atoms.get_potential_energy())
#+END_SRC
#+RESULTS:
:RESULTS:
...
#+RESULTS:
:RESULTS:
kpts= 1 x  1 x 1: Energy = -126.4745. Calc time: 5 min.
kpts= 3 x  3 x 1: Energy = -125.6713. Calc time: 42 min.
kpts= 5 x  5 x 1: Energy = -125.7027. Calc time: 40 min.
kpts= 7 x  7 x 1: Energy = -125.7031. Calc time: 86 min.
kpts= 9 x  9 x 1: Energy = -125.7022. Calc time: 112 min.
kpts=11 x 11 x 1: Energy = -125.7025. Calc time: 166 min.
kpts=13 x 13 x 1: Energy = -125.7024. Calc time: 213 min.
...
for c in cans:
    for pore in c:
        ...
        calc = Vasp(name,
                    ...
                    atoms=a)
#        calc.calculate()
        dtb.result('pore={0}'.format(pname), calc, per_atom=True)
#+END_SRC
...
for c in cans:
    for pore in c:
        ...
        calc = Vasp(name,
                    ...
                    atoms=a)
#        calc.calculate()
        dtb.result('pore={0}'.format(pname), calc)
#+END_SRC
